...
class Discovery(ProcessMining):
    ...
    def get_direct_followers(self, activity_column: str, frequencies=False) -> Set[Path]:
        """
    ...
    @staticmethod
    def get_causal_dependencies(path_set: Set[Path]) -> Set[Path]:
    ...
    def to_json(self, path_set: Set[Path], include_events: bool, output_path=None) -> str:
        """
        ...
        # start & end events
        # TODO concept:name needs to be parameter! Only relevant for completely valid XES files.
        for start in self.process.get_start_activities("Activity"):
            start_events.append({"name": start})

        for end in self.process.get_end_activities("Activity"):
